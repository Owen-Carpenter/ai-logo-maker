-- Create icons table to store user's saved icons
...
CREATE INDEX IF NOT EXISTS icons_tags_idx ON icons USING GIN(tags);
CREATE INDEX IF NOT EXISTS icons_is_favorite_idx ON icons(is_favorite);
...
DROP TRIGGER IF EXISTS update_icons_updated_at_trigger ON icons;
...
-- Create RLS policies (drop existing ones first to make migration idempotent)
